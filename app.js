...
var ws = new WebSocket(process.env.CUSTOMCONNSTR_pushbulletWS);
...
app.use('/notifications', function(req, res, next) {
	var creds = auth(req);
	if(!creds || creds.name !== process.env.CUSTOMCONNSTR_echodroiduser || creds.pass !== process.env.CUSTOMCONNSTR_echodroidpass)
	{
		res.statusCode = 401
